...
def start_process_3(epsilon_value=1.0):
    ...
    if epsilon_value != float(9999):
        command = "CUDA_VISIBLE_DEVICES=2 python3 main_SGD.py --num-clients 5 --epsilon {}".format(epsilon_value)
    else:
        command = "CUDA_VISIBLE_DEVICES=2 python3 main_SGD.py --num-clients 5 --epsilon {} --eta0 {}".format(epsilon_value, 0.01)
...
def start_process_222(num_edge=1):
    # command = "python " + python_file + "  " + str(stepsize) + "  " + str(p_fraction)
    command = "CUDA_VISIBLE_DEVICES=1,2 python3 main_SGD.py --num-clients 10 --p-value 0.1 --epsilon 9999 --dataset 0 --num-edge {}".format(num_edge)
    print(command)
    os.system(command)
