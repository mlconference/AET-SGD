...
FIGURE_PATH = "figure_topology/"
PDF_PATH = "pdf_topology/"
...
# plot_eps = [0.06, 0.1, 1.0, 4.0, 9999]

# list for all files
for fileName in glob.glob("topology/*.json", recursive=True):
    with open(fileName) as json_file:
        json_data = json.load(json_file)
        key_eps = str(json_data['net_topology'])

        ##### New configuration #####
        # if key_eps in plot_eps:
...
for fileName in glob.glob("topology/*.json", recursive=True):
    ...
    with open(fileName) as json_file:
        ...
        if len(result_hashArray[key_eps]) == 0:
            try:
                result_hashArray[key_eps].append(result_arr)
                result_hashArray_Count[key_eps].append(key_eps)

                # list of sample sequence
                # it will be the same for all files with the same eps
                list_sample_sequence = json_data['list_sample_sequence']
                iteration_hashArray[key_eps].append(list_sample_sequence)
            ...
            try:
                result_hashArray[key_eps] = np.add(result_hashArray[key_eps], result_arr)
                result_hashArray_Count[key_eps].append(key_eps)
...
for index in range(len(label_arr)):
    ...
    # temp_epsilon = float(label_arr[index])
    # if temp_epsilon != float(9999): # magic epsilon
    #     ax.plot(geb_b30_x, bar_val, label=("$\epsilon=" + label_arr[index] + "$"))
    # else:
    #     ax.plot(geb_b30_x, bar_val, label=("non-DP"))

    ax.plot(geb_b30_x, bar_val, label=label_arr[index])
...
# f = lambda x, pos: f'{x/10**3:,.0f}K' if x > 0 else f'{x/10**3:,.0f}'
...
file_name = FIGURE_PATH + "topology_" + expt_data_set + "_" + problemType + ".png"
print("fileName={}".format(file_name))
fig.savefig(file_name)

# pdf
file_name_pdf = PDF_PATH + "topology_" + expt_data_set + "_" + problemType + ".pdf"
